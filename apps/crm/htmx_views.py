...
def htmx_stage_leads(request, stage_id):
    ...
    """
    stage = get_object_or_404(Stage, pk=stage_id, organization=request.organization)
...
def htmx_move_lead(request, pk, stage_id):
    ...
    """
    lead = get_object_or_404(Lead, pk=pk, organization=request.organization)
    new_stage = get_object_or_404(Stage, pk=stage_id, organization=request.organization)
    ...
    # Trigger event for updating counters
...
def htmx_delete_lead(request, pk):
    ...
    # Bo'sh response - element o'chiriladi
    return HttpResponse('')

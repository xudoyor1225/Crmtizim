...
def staff_attendance_list(request):
    ...
    # Davomatlar
    attendance_query = StaffAttendance.objects.filter(date=selected_date)
    if org:
        attendance_query = attendance_query.filter(organization=org)

    attendances = attendance_query.select_related('staff')
...
def staff_check_in(request):
    ...
    if request.method == 'POST':
        ...
        att_defaults = {
            'check_in': now,
            'expected_time': staff.profile_data.get('work_start', '09:00') if hasattr(staff, 'profile_data') and staff.profile_data else '09:00',
        }
        if org:
            att_defaults['organization'] = org
...
def staff_check_out(request):
    ...
    if request.method == 'POST':
        ...
        try:
            att_query = StaffAttendance.objects.filter(
                staff=staff,
                date=today
            )
            if org:
                att_query = att_query.filter(organization=org)
